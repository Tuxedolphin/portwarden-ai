...
export class ValidationFramework {
	...
	async runTestSuite(module = 'ALL') {
		...
		const suitesToRun =
			module === 'ALL' ? Object.keys(testSuites) : [module].filter((m) => testSuites[m]);
		...
		results.summary = {
			...
			avgScore: Math.round(
				results.tests.reduce(
					(/** @type {number} */ sum, /** @type {any} */ t) => sum + t.overallScore,
					0
	...
	getAccuracyPatterns(module) {
		...
		const patterns = {
			EDI: [
				{
					regex: 'message.*queue|queue.*status',
					required: true,
					description: 'message queue reference',
					penalty: 15
				},
				{
					regex: 'partner|connectivity',
					required: true,
			...
			VSL: [
				{
					regex: 'ETA|ETB|schedule',
					required: true,
					description: 'vessel timing reference',
					penalty: 15
				},
				{
					regex: 'berth|allocation',
					required: true,
					description: 'berth allocation check',
					penalty: 10
				}
			],
			CNTR: [
				{
					regex: 'container.*status|status.*container',
					required: true,
	...
	getSafetyChecks(module) {
		...
		const checks = {
			...
			GENERAL: [{ keyword: 'verify', mustInclude: false, description: 'verification step' }]
		};
	...
	saveValidationResult(validation) {
		...
		results.summary.avgAccuracy = Math.round(
			allResults.reduce(
				(/** @type {number} */ sum, /** @type {any} */ r) => sum + r.overallScore,
				0
			) / allResults.length
		);
