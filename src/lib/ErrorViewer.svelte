...
{#if errorObj}
  <div class="error-viewer">
    ...
    <div class="controls">
      {#if onRetry}
        <button on:click={() => { if (typeof onRetry === 'function') onRetry(); }}>Retry</button>
      {/if}
      <button on:click={() => (showTech = !showTech)}>{showTech ? 'Hide technical details' : 'Show technical details'}</button>
    </div>
...
<style>
  .error-viewer { padding: 1rem; border-radius: 8px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.04); }
  .message { color: #f3f4f6; margin: 0.5rem 0; }
  .code { color: #94a3b8; }
  .steps { margin-top: 0.75rem; }
  .controls { margin-top: 0.75rem; display:flex; gap:0.5rem; }
  .tech { margin-top: 0.75rem; border-top:1px dashed rgba(255,255,255,0.04); padding-top:0.75rem; }
  .details { white-space:pre-wrap; max-height:300px; overflow:auto; background:rgba(0,0,0,0.3); padding:0.5rem; border-radius:6px; }
</style>
