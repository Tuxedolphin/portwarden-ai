...
<section class="hero-header">
  ...
  <div class="archive-stats">
    ...
    <div class="stat-card">
      <span class="stat-value">{items.filter(i => new Date(i.archived_at) > new Date(Date.now() - 7*24*60*60*1000)).length}</span>
      <span class="stat-label">This Week</span>
    </div>
    <div class="stat-card">
      <span class="stat-value">{items.filter(i => new Date(i.archived_at) > new Date(Date.now() - 30*24*60*60*1000)).length}</span>
    ...
    {#if items.length === 0}
      <div class="empty-archive">
        ...
        {#if q || tagInput}
          ...
          <button class="clear-search" on:click={() => { q = ''; tagInput = ''; load(); }}>
            Clear Search
          </button>
        {:else}
